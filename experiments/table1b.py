...
basicops = ["relu6", "abs", "sigmoid", "tanh", "swish"]
gen_time = dict()
...
for c in certifiers:
    ...
    for b in basicops:
        ...
        ret_dict_correct = run_experiment_correct(folder + c + "_" + b, 1, 1)
        print()
        print(f"Running {c} on unsound {b}")
        ret_dict_incorrect = run_experiment_incorrect(folder + c + "_" + b, 1, 1)
        print()
        print()
...
print(f"(b) New DNN operations introduced in § 6.2")
